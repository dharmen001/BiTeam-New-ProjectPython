...
class Summary():
    ...
    def connect_TFR_summary(self):
        ...
        #sql_VDX_summary = "select substr(PLACEMENT_DESC,1,INSTR(PLACEMENT_DESC, '.', 1)-1) "'Placement#'", SDATE "'Start_Date'", EDATE "'End_Date'", CREATIVE_DESC "'Placement_Name'" , COST_TYPE_DESC "'Cost_type'" ,UNIT_COST "'Unit_Cost'", BUDGET "'Planned_Cost'", BOOKED_QTY "'Booked_Imp#Booked_Eng'" from TFR_REP.SUMMARY_MV where (IO_ID = {}) AND (DATA_SOURCE = 'KM') AND (CREATIVE_DESC NOT IN('Pre-roll - Desktop','Pre-roll - Desktop + Mobile','pre-roll – desktop + mobile','Pre-roll - In-stream/Mobile Blend','Pre-roll - Mobile','Pre-roll -Desktop','Pre-roll - In-stream')) ORDER BY PLACEMENT_ID".format(self.config.IO_ID)
        ...
        sql_Display_MV = "select substr(PLACEMENT_DESC,1,INSTR(PLACEMENT_DESC, '.', 1)-1) as "'Placement#'", sum(VIEWS) as "'Delivered_Impresion'", sum(CLICKS) as "'Clicks'", sum(CONVERSIONS) as "'Conversion'" from TFR_REP.DAILY_SALES_MV WHERE IO_ID = {} GROUP BY PLACEMENT_ID, PLACEMENT_DESC ORDER BY PLACEMENT_ID".format(self.config.IO_ID)
    ...
    def access_data_summary(self):
        ...
        preroll_third_summary = read_sql_preroll.merge(read_sql_preroll_mv, on = "PLACEMENT#", how="inner", suffixes=('_1','_2'))


        preroll_third_table = preroll_third_summary[["PLACEMENT#","START_DATE","END_DATE","PLACEMENT_NAME","COST_TYPE",
                                                     "UNIT_COST","PLANNED_COST","BOOKED_IMP#BOOKED_ENG","IMPRESSION","COMPLETIONS"]]

        conditions_CPCV = [(preroll_third_table.loc[:,['COST_TYPE']] == 'CPCV')]
        choices_CPCV = [preroll_third_table.loc[:,['COMPLETIONS']]]

        try:
            preroll_third_table['Delivered_Impressions']= np.select(conditions_CPCV,choices_CPCV,default = preroll_third_table.loc[:,["IMPRESSION"]])
        except ValueError as e:
            pass

        try:
            preroll_access_table = preroll_third_table[["PLACEMENT#","START_DATE","END_DATE","PLACEMENT_NAME","COST_TYPE",
                                                     "UNIT_COST","PLANNED_COST","BOOKED_IMP#BOOKED_ENG","Delivered_Impressions"]]
        except KeyError as e:
    ...
    def summary_creation(self):
        ...
        try:
            display_first_Table['Delivery%'] = display_first_Table['DELIVERED_IMPRESION']/display_first_Table['BOOKED_IMP#BOOKED_ENG']
        except KeyError as e:
            pass
        try:
            display_first_Table['Spend'] = display_first_Table['DELIVERED_IMPRESION']/1000*display_first_Table['UNIT_COST']
        except KeyError as e:
            pass
        ...
        try:
            ...
            vdx_access_table['Spend']=np.select([mask1,mast2],[choices_vdx_eng_spend,choices_vdx_cpcv_spend],default=0.00)
        except KeyError as e:
            pass
        ...
        try:
          preroll_access_table['Delivery%'] = preroll_access_table["Delivered_Impressions"]/preroll_access_table["BOOKED_IMP#BOOKED_ENG"]
          preroll_access_table['Spend'] = preroll_access_table["Delivered_Impressions"]/1000*preroll_access_table["UNIT_COST"]
        except KeyError as e:
            pass
    ...
    def rename_cols_sumary(self):
        ...
        vdx_rename=vdx_access_table.rename(columns={"PLACEMENT#":"Placement#","START_DATE":"Start Date","END_DATE":"End Date",
                                                         "PLACEMENT_NAME":"Placement Name",
                                                         "COST_TYPE":"Cost Type","UNIT_COST":"Unit Cost",
                                                         "PLANNED_COST":"Planned Cost","BOOKED_IMP#BOOKED_ENG":"Booked"},
                                                inplace=True)

        preroll_rename=preroll_access_table.rename(columns={"PLACEMENT#":"Placement#","START_DATE":"Start Date","END_DATE":"End Date",
                                                         "PLACEMENT_NAME":"Placement Name",
                                                         "COST_TYPE":"Cost Type","UNIT_COST":"Unit Cost",
                                                         "PLANNED_COST":"Planned Cost","BOOKED_IMP#BOOKED_ENG":"Booked"},
                                                inplace=True)
    ...
    def write_summary(self):
        ...
        if check_disp_empty == True:
            ...
            display_write = display_first_Table.to_excel(self.config.writer,sheet_name="Summary({})".format(self.config.IO_ID),
        ...
        if check_preroll_empty == True:
            ...
            preroll_write = preroll_access_table.to_excel(self.config.writer,sheet_name="Summary({})".format(self.config.IO_ID),
                                                     startcol=3,startrow=len(display_first_Table)+len(vdx_access_table)+18,
                                                     header=True,index=False)
    ...
    def format_summary(self):
        ...
        worksheet.insert_image("H2","Exponential.png",{"url":"https://www.tribalfusion.com/adapp/login/expo9"})
        worksheet.insert_image("K2","Client_Logo.png")
        ...
        format_subtotal = workbook.add_format({"bold":True,"align":"centre"})
        format_sub_num_money = workbook.add_format({"bold":True,"num_format":"$#,###0.00","align":"right"})
        ...
        worksheet.conditional_format(number_rows_commom+number_rows_display+number_rows_vdx+16,12,
                              number_rows_commom+number_rows_display+number_rows_vdx+number_rows_preroll+15,12,
                                     {"type":"no_blanks","format":percent_fmt})
        worksheet.conditional_format(number_rows_commom+number_rows_display+number_rows_vdx+16,13,
                                     number_rows_commom+number_rows_display+number_rows_vdx+number_rows_preroll+15,13,
                                     {"type":"no_blanks","format":money_fmt})
        worksheet.conditional_format(number_rows_commom+number_rows_display+number_rows_vdx+16,8,
                                     number_rows_commom+number_rows_display+number_rows_vdx+number_rows_preroll+15,9,
                                     {"type":"no_blanks","format":money_fmt})
        ...
        if  check_preroll_empty == True:
            ...
            worksheet.write(number_rows_commom+number_rows_display+number_rows_vdx+number_rows_preroll+16,3,"Subtotal",format_subtotal)

        if check_disp_empty==True:
            pass
        else:
            worksheet.write_formula(number_rows_commom+number_rows_display+6,9,
                                    '=sum(J{}:J{})'.format(number_rows_commom+7,number_rows_commom+number_rows_display+6),format_sub_num_money)
            worksheet.write_formula(number_rows_commom+number_rows_display+6,10,
                                    '=sum(K{}:K{})'.format(number_rows_commom+7,number_rows_commom+number_rows_display+6),format_sub)
            worksheet.write_formula(number_rows_commom+number_rows_display+6,11,
                                    '=sum(L{}:L{})'.format(number_rows_commom+7,number_rows_commom+number_rows_display+6),format_sub)
            worksheet.write_formula(number_rows_commom+number_rows_display+6,12,
                                    '=(L{}/K{})'.format(number_rows_commom+number_rows_display+7,number_rows_commom+number_rows_display+7),format_sub_num_percent)
            worksheet.write_formula(number_rows_commom+number_rows_display+6,13,
                                    '=sum(N{}:N{})'.format(number_rows_commom+7,number_rows_commom+number_rows_display+6),format_sub_num_money)

        if check_vdx_empty == True:
            pass
        else:
            worksheet.write_formula(number_rows_commom+number_rows_display+number_rows_vdx+11,9,
                                    '=sum(J{}:J{})'.format(number_rows_commom+number_rows_display+12,number_rows_commom+number_rows_display+number_rows_vdx+11),
                                    format_sub_num_money)
            worksheet.write_formula(number_rows_commom+number_rows_display+number_rows_vdx+11,10,
                                    '=sum(K{}:K{})'.format(number_rows_commom+number_rows_display+12,number_rows_commom+number_rows_display+number_rows_vdx+11),
                                    format_sub)
            worksheet.write_formula(number_rows_commom+number_rows_display+number_rows_vdx+11,11,
                                    '=sum(L{}:L{})'.format(number_rows_commom+number_rows_display+12,number_rows_commom+number_rows_display+number_rows_vdx+11),
                                    format_sub)
            worksheet.write_formula(number_rows_commom+number_rows_display+number_rows_vdx+11,12,
                                    '=sum(M{}:M{})'.format(number_rows_commom+number_rows_display+12,number_rows_commom+number_rows_display+number_rows_vdx+11),format_sub)
            worksheet.write_formula(number_rows_commom+number_rows_display+number_rows_vdx+11,13,
                                    '=sum(L{}:M{})/sum(K{})'.format(number_rows_commom+number_rows_display+number_rows_vdx+12,number_rows_commom+number_rows_display+number_rows_vdx+12,number_rows_commom+number_rows_display+number_rows_vdx+12),
        ...
        if check_preroll_empty == True:
            ...
            worksheet.write_formula(number_rows_commom+number_rows_display+number_rows_vdx+number_rows_preroll+16,9,
                                    '=sum(J{}:J{})'.format(number_rows_commom+number_rows_display+number_rows_vdx+17,number_rows_commom+number_rows_display+number_rows_vdx+number_rows_preroll+16),
                                    format_sub_num_money)
            worksheet.write_formula(number_rows_commom+number_rows_display+number_rows_vdx+number_rows_preroll+16,10,
                                    '=sum(K{}:K{})'.format(number_rows_commom+number_rows_display+number_rows_vdx+17,number_rows_commom+number_rows_display+number_rows_vdx+number_rows_preroll+16),
                                    format_sub)
            worksheet.write_formula(number_rows_commom+number_rows_display+number_rows_vdx+number_rows_preroll+16,11,
                                    '=sum(L{}:L{})'.format(number_rows_commom+number_rows_display+number_rows_vdx+17,number_rows_commom+number_rows_display+number_rows_vdx+number_rows_preroll+16),
                                    format_sub)
            worksheet.write_formula(number_rows_commom+number_rows_display+number_rows_vdx+number_rows_preroll+16,12,
                                    '=IFERROR((L{}/K{}),0)'.format(number_rows_commom+number_rows_display+number_rows_vdx+number_rows_preroll+17,
                                                        number_rows_commom+number_rows_display+number_rows_vdx+number_rows_preroll+17),format_sub_num_percent)
            worksheet.write_formula(number_rows_commom+number_rows_display+number_rows_vdx+number_rows_preroll+16,13,
                                    '=sum(N{}:N{})'.format(number_rows_commom+number_rows_display+number_rows_vdx+17,number_rows_commom+number_rows_display+number_rows_vdx+number_rows_preroll+16),
                                    format_sub_num_money)
        ...
        if check_disp_empty == True:
            ...
            worksheet.conditional_format(number_rows_commom+number_rows_display+6,3,number_rows_commom+number_rows_display+6
        ...
        if check_preroll_empty == True:
            ...
            worksheet.conditional_format(number_rows_commom+number_rows_display+number_rows_vdx+number_rows_preroll+16,3,
        ...
        #worksheet.conditional_format("N{}:N{}".format(number_rows_commom+6,number_rows_commom+number_cols_display+7),
        ...
        if check_vdx_empty == True:
            ...
            worksheet.merge_range(number_rows_commom+number_rows_display+11,1,number_rows_commom+number_rows_display+number_rows_vdx+11,2,
